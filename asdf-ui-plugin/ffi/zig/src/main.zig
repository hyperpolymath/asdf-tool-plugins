...
const VERSION = "0.1.0";
const BUILD_INFO = "UI built with Zig " ++ @import("builtin").zig_version_string;
...
export fn ui_free(handle: ?*Handle) void {
...
export fn ui_process(handle: ?*Handle, input: u32) Result {
...
export fn ui_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn ui_free_string(str: ?[*:0]const u8) void {
...
export fn ui_last_error() ?[*:0]const u8 {
...
export fn ui_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn ui_build_info() [*:0]const u8 {
...
/// Register a callback
export fn ui_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn ui_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(ui_is_initialized(handle) == 1);
}

test "error handling" {
    const result = ui_process(null, 0);

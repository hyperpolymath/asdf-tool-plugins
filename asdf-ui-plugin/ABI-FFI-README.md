...
│       └── include/
│           └── ui.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── ui.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/ui.h
```
...
int main() {
    ...
    int result = ui_process(handle, 42);
    if (result != 0) {
        const char* err = ui_last_error();
...
Compile with:
```bash
gcc -o example example.c -lui -L./zig-out/lib
```

### From Idris2

```idris
import UI.ABI.Foreign
...
#[link(name = "ui")]
extern "C" {
    fn ui_init() -> *mut std::ffi::c_void;
    fn ui_free(handle: *mut std::ffi::c_void);
    fn ui_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}
...
fn main() {
    unsafe {
        ...
        let result = ui_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:ui_init, libui), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:ui_process, libui), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:ui_free, libui), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/ui.h
   ```

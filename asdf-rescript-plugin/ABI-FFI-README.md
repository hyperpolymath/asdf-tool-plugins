...
int main() {
    ...
    int result = rescript_process(handle, 42);
    if (result != 0) {
        const char* err = rescript_last_error();
...
Compile with:
```bash
gcc -o example example.c -lrescript -L./zig-out/lib
```
...
extern "C" {
    ...
    fn rescript_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = rescript_init();
        assert!(!handle.is_null());

        let result = rescript_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const librescript = "librescript"

function init()
    handle = ccall((:rescript_init, librescript), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:rescript_process, librescript), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:rescript_free, librescript), Cvoid, (Ptr{Cvoid},), handle)
end

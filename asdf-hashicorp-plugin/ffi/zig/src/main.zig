...
const VERSION = "0.1.0";
const BUILD_INFO = "HASHICORP built with Zig " ++ @import("builtin").zig_version_string;
...
export fn hashicorp_init() ?*Handle {
...
export fn hashicorp_free(handle: ?*Handle) void {
...
export fn hashicorp_process(handle: ?*Handle, input: u32) Result {
...
export fn hashicorp_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn hashicorp_free_string(str: ?[*:0]const u8) void {
...
export fn hashicorp_process_array(
    handle: ?*Handle,
    buffer: ?[*]const u8,
    len: u32,
) Result {
...
export fn hashicorp_last_error() ?[*:0]const u8 {
...
export fn hashicorp_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn hashicorp_build_info() [*:0]const u8 {
...
/// Register a callback
export fn hashicorp_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn hashicorp_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(hashicorp_is_initialized(handle) == 1);
}

test "error handling" {
    const result = hashicorp_process(null, 0);

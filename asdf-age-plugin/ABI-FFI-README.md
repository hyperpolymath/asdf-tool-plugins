...
│       └── include/
│           └── age.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── age.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/age.h
```
...
int main() {
    void* handle = age_init();
    if (!handle) return 1;

    int result = age_process(handle, 42);
    if (result != 0) {
        const char* err = age_last_error();
...
Compile with:
```bash
gcc -o example example.c -lage -L./zig-out/lib
```

### From Idris2

```idris
import AGE.ABI.Foreign
...
#[link(name = "age")]
extern "C" {
    fn age_init() -> *mut std::ffi::c_void;
    fn age_free(handle: *mut std::ffi::c_void);
    fn age_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}
...
fn main() {
    unsafe {
        ...
        let result = age_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:age_init, libage), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:age_process, libage), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:age_free, libage), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/age.h
   ```

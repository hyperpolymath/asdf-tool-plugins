...
│       └── include/
│           └── doctl.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── doctl.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/doctl.h
```
...
int main() {
    void* handle = doctl_init();
    if (!handle) return 1;

    int result = doctl_process(handle, 42);
    if (result != 0) {
        const char* err = doctl_last_error();
...
Compile with:
```bash
gcc -o example example.c -ldoctl -L./zig-out/lib
```

### From Idris2

```idris
import DOCTL.ABI.Foreign
...
#[link(name = "doctl")]
extern "C" {
    fn doctl_init() -> *mut std::ffi::c_void;
    fn doctl_free(handle: *mut std::ffi::c_void);
    fn doctl_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = doctl_init();
        assert!(!handle.is_null());

        let result = doctl_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libdoctl = "libdoctl"

function init()
    handle = ccall((:doctl_init, libdoctl), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:doctl_process, libdoctl), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:doctl_free, libdoctl), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/doctl.h
   ```

...
const VERSION = "0.1.0";
const BUILD_INFO = "YQ built with Zig " ++ @import("builtin").zig_version_string;
...
export fn yq_free(handle: ?*Handle) void {
...
export fn yq_process(handle: ?*Handle, input: u32) Result {
...
export fn yq_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn yq_free_string(str: ?[*:0]const u8) void {
...
export fn yq_last_error() ?[*:0]const u8 {
...
export fn yq_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn yq_build_info() [*:0]const u8 {
...
/// Register a callback
export fn yq_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn yq_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(yq_is_initialized(handle) == 1);
}

test "error handling" {
    const result = yq_process(null, 0);

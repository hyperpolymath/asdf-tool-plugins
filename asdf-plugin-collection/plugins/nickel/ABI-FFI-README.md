...
│       └── include/
│           └── nickel.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── nickel.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/nickel.h
```
...
int main() {
    void* handle = nickel_init();
    if (!handle) return 1;

    int result = nickel_process(handle, 42);
    if (result != 0) {
        const char* err = nickel_last_error();
...
Compile with:
```bash
gcc -o example example.c -lnickel -L./zig-out/lib
```

### From Idris2

```idris
import NICKEL.ABI.Foreign
...
#[link(name = "nickel")]
extern "C" {
    fn nickel_init() -> *mut std::ffi::c_void;
    fn nickel_free(handle: *mut std::ffi::c_void);
    fn nickel_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = nickel_init();
        assert!(!handle.is_null());

        let result = nickel_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libnickel = "libnickel"

function init()
    handle = ccall((:nickel_init, libnickel), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:nickel_process, libnickel), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:nickel_free, libnickel), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/nickel.h
   ```

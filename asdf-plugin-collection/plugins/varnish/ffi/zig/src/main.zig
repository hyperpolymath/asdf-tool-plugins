...
const VERSION = "0.1.0";
const BUILD_INFO = "VARNISH built with Zig " ++ @import("builtin").zig_version_string;
...
export fn varnish_free(handle: ?*Handle) void {
...
export fn varnish_process(handle: ?*Handle, input: u32) Result {
...
export fn varnish_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn varnish_free_string(str: ?[*:0]const u8) void {
...
export fn varnish_process_array(
    handle: ?*Handle,
    buffer: ?[*]const u8,
    len: u32,
) Result {
...
export fn varnish_last_error() ?[*:0]const u8 {
...
export fn varnish_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn varnish_build_info() [*:0]const u8 {
...
/// Register a callback
export fn varnish_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn varnish_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(varnish_is_initialized(handle) == 1);
}

test "error handling" {
    const result = varnish_process(null, 0);

...
const VERSION = "0.1.0";
const BUILD_INFO = "METAICONIC built with Zig " ++ @import("builtin").zig_version_string;
...
export fn metaiconic_init() ?*Handle {
...
export fn metaiconic_free(handle: ?*Handle) void {
...
export fn metaiconic_process(handle: ?*Handle, input: u32) Result {
...
export fn metaiconic_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn metaiconic_free_string(str: ?[*:0]const u8) void {
...
export fn metaiconic_process_array(
    handle: ?*Handle,
    buffer: ?[*]const u8,
    len: u32,
) Result {
...
export fn metaiconic_last_error() ?[*:0]const u8 {
...
export fn metaiconic_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn metaiconic_build_info() [*:0]const u8 {
...
/// Register a callback
export fn metaiconic_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn metaiconic_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(metaiconic_is_initialized(handle) == 1);
}

test "error handling" {
    const result = metaiconic_process(null, 0);

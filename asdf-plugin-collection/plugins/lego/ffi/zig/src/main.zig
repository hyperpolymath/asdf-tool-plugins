...
const VERSION = "0.1.0";
const BUILD_INFO = "LEGO built with Zig " ++ @import("builtin").zig_version_string;
...
export fn lego_free(handle: ?*Handle) void {
...
export fn lego_process(handle: ?*Handle, input: u32) Result {
...
export fn lego_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn lego_free_string(str: ?[*:0]const u8) void {
...
export fn lego_last_error() ?[*:0]const u8 {
...
export fn lego_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn lego_build_info() [*:0]const u8 {
...
/// Register a callback
export fn lego_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn lego_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(lego_is_initialized(handle) == 1);
}

test "error handling" {
    const result = lego_process(null, 0);

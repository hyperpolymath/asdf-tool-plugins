    ...
    V: Clone,
{
    ...
    pub fn get(&self, key: &K) -> Result<V> {
        let mut cache = self.cache.lock().expect("SPARK-candidate: mutex_no_poisoning — cache lock is never held across a panic boundary");
    ...
    pub fn len(&self) -> usize {
        let cache = self.cache.lock().expect("SPARK-candidate: mutex_no_poisoning — cache lock is never held across a panic boundary");
        cache.len()
    }

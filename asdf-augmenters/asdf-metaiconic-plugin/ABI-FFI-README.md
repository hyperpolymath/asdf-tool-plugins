...
-->
# METAICONIC ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libmetaiconic.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── metaiconic.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── metaiconic.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/metaiconic.h
```
...
```c
#include "metaiconic.h"

int main() {
    void* handle = metaiconic_init();
    if (!handle) return 1;

    int result = metaiconic_process(handle, 42);
    if (result != 0) {
        const char* err = metaiconic_last_error();
...
Compile with:
```bash
gcc -o example example.c -lmetaiconic -L./zig-out/lib
```

### From Idris2

```idris
import METAICONIC.ABI.Foreign
...
```rust
#[link(name = "metaiconic")]
extern "C" {
    fn metaiconic_init() -> *mut std::ffi::c_void;
    fn metaiconic_free(handle: *mut std::ffi::c_void);
    fn metaiconic_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = metaiconic_init();
        assert!(!handle.is_null());

        let result = metaiconic_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libmetaiconic = "libmetaiconic"

function init()
    handle = ccall((:metaiconic_init, libmetaiconic), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:metaiconic_process, libmetaiconic), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:metaiconic_free, libmetaiconic), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/metaiconic.h
   ```

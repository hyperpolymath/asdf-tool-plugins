...
│       └── include/
│           └── borg.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── borg.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/borg.h
```
...
int main() {
    void* handle = borg_init();
    if (!handle) return 1;

    int result = borg_process(handle, 42);
    if (result != 0) {
        const char* err = borg_last_error();
...
Compile with:
```bash
gcc -o example example.c -lborg -L./zig-out/lib
```

### From Idris2

```idris
import BORG.ABI.Foreign
...
#[link(name = "borg")]
extern "C" {
    fn borg_init() -> *mut std::ffi::c_void;
    fn borg_free(handle: *mut std::ffi::c_void);
    fn borg_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = borg_init();
        assert!(!handle.is_null());

        let result = borg_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libborg = "libborg"

function init()
    handle = ccall((:borg_init, libborg), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:borg_process, libborg), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:borg_free, libborg), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/borg.h
   ```

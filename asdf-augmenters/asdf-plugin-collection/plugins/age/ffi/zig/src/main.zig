...
const VERSION = "0.1.0";
const BUILD_INFO = "AGE built with Zig " ++ @import("builtin").zig_version_string;
...
export fn age_free(handle: ?*Handle) void {
...
export fn age_process(handle: ?*Handle, input: u32) Result {
...
export fn age_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn age_free_string(str: ?[*:0]const u8) void {
...
export fn age_last_error() ?[*:0]const u8 {
...
export fn age_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn age_build_info() [*:0]const u8 {
...
/// Register a callback
export fn age_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn age_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(age_is_initialized(handle) == 1);
}

test "error handling" {
    const result = age_process(null, 0);

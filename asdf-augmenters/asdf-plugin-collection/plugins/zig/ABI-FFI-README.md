...
│       └── include/
│           └── zig.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── zig.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/zig.h
```
...
int main() {
    void* handle = zig_init();
    if (!handle) return 1;

    int result = zig_process(handle, 42);
    if (result != 0) {
        const char* err = zig_last_error();
...
Compile with:
```bash
gcc -o example example.c -lzig -L./zig-out/lib
```

### From Idris2

```idris
import ZIG.ABI.Foreign
...
#[link(name = "zig")]
extern "C" {
    fn zig_init() -> *mut std::ffi::c_void;
    fn zig_free(handle: *mut std::ffi::c_void);
    fn zig_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}
...
fn main() {
    unsafe {
        ...
        let result = zig_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:zig_init, libzig), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:zig_process, libzig), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:zig_free, libzig), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/zig.h
   ```

...
# INFLUXDB ABI/FFI Documentation
...
│       └── include/
│           └── influxdb.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── influxdb.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/influxdb.h
```
...
int main() {
    void* handle = influxdb_init();
    if (!handle) return 1;

    int result = influxdb_process(handle, 42);
    if (result != 0) {
        const char* err = influxdb_last_error();
...
Compile with:
```bash
gcc -o example example.c -linfluxdb -L./zig-out/lib
```

### From Idris2

```idris
import INFLUXDB.ABI.Foreign
...
#[link(name = "influxdb")]
extern "C" {
    fn influxdb_init() -> *mut std::ffi::c_void;
    fn influxdb_free(handle: *mut std::ffi::c_void);
    fn influxdb_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = influxdb_init();
        assert!(!handle.is_null());

        let result = influxdb_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libinfluxdb = "libinfluxdb"

function init()
    handle = ccall((:influxdb_init, libinfluxdb), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:influxdb_process, libinfluxdb), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:influxdb_free, libinfluxdb), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/influxdb.h
   ```

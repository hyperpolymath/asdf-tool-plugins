...
│       └── include/
│           └── ada.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── ada.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/ada.h
```
...
int main() {
    void* handle = ada_init();
    if (!handle) return 1;

    int result = ada_process(handle, 42);
    if (result != 0) {
        const char* err = ada_last_error();
...
Compile with:
```bash
gcc -o example example.c -lada -L./zig-out/lib
```

### From Idris2

```idris
import ADA.ABI.Foreign
...
#[link(name = "ada")]
extern "C" {
    fn ada_init() -> *mut std::ffi::c_void;
    fn ada_free(handle: *mut std::ffi::c_void);
    fn ada_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}
...
fn main() {
    unsafe {
        ...
        let result = ada_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:ada_init, libada), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:ada_process, libada), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:ada_free, libada), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/ada.h
   ```

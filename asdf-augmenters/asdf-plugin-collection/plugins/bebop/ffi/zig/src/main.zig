...
const VERSION = "0.1.0";
const BUILD_INFO = "BEBOP built with Zig " ++ @import("builtin").zig_version_string;
...
export fn bebop_free(handle: ?*Handle) void {
...
export fn bebop_process(handle: ?*Handle, input: u32) Result {
...
export fn bebop_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn bebop_free_string(str: ?[*:0]const u8) void {
...
export fn bebop_last_error() ?[*:0]const u8 {
...
export fn bebop_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn bebop_build_info() [*:0]const u8 {
...
/// Register a callback
export fn bebop_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn bebop_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(bebop_is_initialized(handle) == 1);
}

test "error handling" {
    const result = bebop_process(null, 0);

...
│       └── include/
│           └── dhall.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── dhall.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/dhall.h
```
...
int main() {
    void* handle = dhall_init();
    if (!handle) return 1;

    int result = dhall_process(handle, 42);
    if (result != 0) {
        const char* err = dhall_last_error();
...
Compile with:
```bash
gcc -o example example.c -ldhall -L./zig-out/lib
```

### From Idris2

```idris
import DHALL.ABI.Foreign
...
#[link(name = "dhall")]
extern "C" {
    fn dhall_init() -> *mut std::ffi::c_void;
    fn dhall_free(handle: *mut std::ffi::c_void);
    fn dhall_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = dhall_init();
        assert!(!handle.is_null());

        let result = dhall_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libdhall = "libdhall"

function init()
    handle = ccall((:dhall_init, libdhall), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:dhall_process, libdhall), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:dhall_free, libdhall), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/dhall.h
   ```

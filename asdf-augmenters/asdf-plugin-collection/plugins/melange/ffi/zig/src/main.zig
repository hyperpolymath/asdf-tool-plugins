...
const VERSION = "0.1.0";
const BUILD_INFO = "MELANGE built with Zig " ++ @import("builtin").zig_version_string;
...
export fn melange_free(handle: ?*Handle) void {
...
export fn melange_process(handle: ?*Handle, input: u32) Result {
...
export fn melange_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn melange_free_string(str: ?[*:0]const u8) void {
...
export fn melange_process_array(
    handle: ?*Handle,
    buffer: ?[*]const u8,
    len: u32,
) Result {
...
export fn melange_last_error() ?[*:0]const u8 {
...
export fn melange_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn melange_build_info() [*:0]const u8 {
...
/// Register a callback
export fn melange_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn melange_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(melange_is_initialized(handle) == 1);
}

test "error handling" {
    const result = melange_process(null, 0);

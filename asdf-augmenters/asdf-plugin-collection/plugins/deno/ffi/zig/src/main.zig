...
const VERSION = "0.1.0";
const BUILD_INFO = "DENO built with Zig " ++ @import("builtin").zig_version_string;
...
export fn deno_free(handle: ?*Handle) void {
...
export fn deno_process(handle: ?*Handle, input: u32) Result {
...
export fn deno_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn deno_free_string(str: ?[*:0]const u8) void {
...
export fn deno_last_error() ?[*:0]const u8 {
...
export fn deno_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn deno_build_info() [*:0]const u8 {
...
/// Register a callback
export fn deno_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn deno_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(deno_is_initialized(handle) == 1);
}

test "error handling" {
    const result = deno_process(null, 0);

...
│       └── include/
│           └── mariadb.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── mariadb.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/mariadb.h
```
...
int main() {
    void* handle = mariadb_init();
    if (!handle) return 1;

    int result = mariadb_process(handle, 42);
    if (result != 0) {
        const char* err = mariadb_last_error();
...
Compile with:
```bash
gcc -o example example.c -lmariadb -L./zig-out/lib
```

### From Idris2

```idris
import MARIADB.ABI.Foreign
...
#[link(name = "mariadb")]
extern "C" {
    fn mariadb_init() -> *mut std::ffi::c_void;
    fn mariadb_free(handle: *mut std::ffi::c_void);
    fn mariadb_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = mariadb_init();
        assert!(!handle.is_null());

        let result = mariadb_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libmariadb = "libmariadb"

function init()
    handle = ccall((:mariadb_init, libmariadb), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:mariadb_process, libmariadb), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:mariadb_free, libmariadb), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/mariadb.h
   ```
